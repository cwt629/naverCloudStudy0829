...
public class Ex1_ComboBox extends JFrame{
	...
	private void setDesign() {
		...
		// 상단에 추가
		//this.add("North", combo); // 라인 전체에 걸치는 선택박스
		JPanel pTop = new JPanel();
		pTop.add(combo);
		this.add("North", pTop); // 가운데 짧게 그려지는 선택박스
		...
		// Center에는 JPanel p 추가
		p.setBackground(Color.red);
		this.add("Center", p);
		
		// 콤보박스 이벤트 : addItemListener
		combo.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				//System.out.println(selectColor); // 2번씩 출력되는 현상 있음
				
				// jdk8부터 switch에서 String도 가능해짐
				switch(selectColor)
				{
				case "빨강":
					p.setBackground(Color.red);
