...
-- num에 null값을 주면 null이 들어가는 게 아니라 autoincrement된 숫자가 그대로 발생된다
...
-- sawon의 name에 '이'가 포함되어 있는 데이터 조회
select * from sawon where name like concat('%','이','%');
select replace('Happy Day!!', 'Happy', 'Nice') from dual; -- Happy -> Nice로 변경해서 출력
select instr('Happy Day', 'Day') from dual; -- 7 : Day의 위치 찾기(시작은 1부터)
...
select mid('Have a Nice Day', 6, 6) from dual; -- 6번부터 6글자 추출 : a Nice
select substring('Have a Nice Day', 6, 6) from dual; -- 6번부터 6글자 추출 : a Nice
...
select truncate(2.345, 2) from dual; -- 소수점 2자리까지 출력, 무조건 내림
select mod(7,2) from dual; -- 나머지

-- Group by
-- 부서별 인원수, 최고점수, 최저점수, 평균점수 구함
...
-- sub table에 데이터가 추가된 상태에서 부모 테이블에 데이터를 삭제해보자
